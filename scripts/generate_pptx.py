...
新增第 12 页「宝妈绝境案例」：真跑真数据（scripts/mom_comeback_sim.py），
诚实呈现"没翻盘=赛道错配"，严禁讲成翻盘/拿offer的励志故事——
...
text_page("绝境案例：一位宝妈的真实推演（真数据 · 未美化）",[             # 12 诚实反例：赛道错配
 "画像：28岁 / 双非本科文科 / 2年全职带娃空窗 / 目标项目管理——在约300家公司大市场真跑，每档各8次（共24次真实LLM沙盘）",
 "① 基线（原始简历）：平均投52份 → 0面试 · 0 offer（8/8全军覆没）",
 "② 反事实·纯包装（带娃重构为多线程管理+主攻创业公司+薪资降10%）：仍 0 offer（8/8）——包装骗不过硬维度筛选",
 "③ 反事实·真补实力（PMP拿证+两个真实项目，含金量25→50）：本赛道仍 0 offer（8/8）",
 "真相：瓶颈不是简历措辞，是赛道错配——该市场307家里263家以技术岗打头、57%要求硕士，双非文科转岗天然够不着门槛",
 "沙盘不是用来证明「努力就能翻盘」的，而是提前告诉你残酷真相：真正的后悔药是趁早换一条能上桌的赛道，而非在错误赛道里死磕包装",
],accent=GOLD)
