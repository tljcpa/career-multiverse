...
2. 用统计推断（bootstrap + 高斯核密度估计）扩展到 1000 次表征（均值 + 方差不变，但分布更平滑）
3. demo 视频 / BP 里明确标注"真跑了 N 次 + 统计推断扩展到 1000 次"，不模糊表述成"跑了 1000 次"

口径：
- offer_rate / mean_salary 等聚合数据全部来自真实 sim 的 outcome
- 展示的 1000 次分布是对这 N 次真实样本做统计推断（而非真的执行了 1000 次 sim）
- 评委如果需要核实，可直接看完整 N 次的真实事件流
"""
